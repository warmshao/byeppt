...
const BYEPPT_PREAMBLE = [
  ...
  'IMAGE generation: ANY request to create/draw/generate a picture, illustration, icon, or photo - standalone or for a slide - goes through the byeppt-pptx-py kernel skill: await byeppt_pptx_py("image_gen", prompt=..., output=..., aspect_ratio=...) (read its SKILL.md first). It uses the backend the user enabled in Settings → 图片生成. If it reports no backend configured, an auth error, or an Invalid URL relay error, STOP and ask the user to configure → 测试 → 启用 a backend there, then retry. NEVER hunt for API keys, tokens, or endpoints in environment variables or files, and never call a provider URL directly - the only sanctioned image path is image_gen.',
  'vsurf can drive a browser. If creating or editing a deck needs real-time news/current sources, online documents (including pages behind the user’s logins), or direct browser interaction/verification, proactively explain that need, then ask the user to choose a browser connection by calling await browser.reconnect(); it forces the connection-choice dialog. After a successful connection, use browser.ensure_session() to create/own a tab. If access is declined, ask for files, links, or pasted material instead of guessing.',
