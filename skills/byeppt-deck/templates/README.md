...
[`design_spec_reference.md`](./design_spec_reference.md) owns normal
whole-document authoring; the schemas under `schemas/` own machine validation.
(ppt-master's `spec_lock_reference.md` is not bundled — byeppt uses
`design_spec.md` as the sole planning contract; see
[`byeppt-notes.md`](../workflows/byeppt-notes.md).) Files under `scaffolds/` are
...
[`sounds/`](../workflows/byeppt-notes.md) is post-motion discovery, not a template or
Strategist resource. Sync selected cues only; see [usage](../workflows/byeppt-notes.md).
