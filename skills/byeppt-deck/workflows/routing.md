...
| Existing generated project needs a deck-wide `colors.*` or universal `typography.font_family` substitution | Stay in Generate; load [`update_spec.py`](../../byeppt-pptx-py/src/byeppt_pptx_py/scripts/docs/update_spec.md), honor its supported-key boundary, then rerun the final quality gate and Step 7 export |
